...
export interface StatsByGewerk { gewerk: string; done?: number; in_progress?: number; offen?: number; }
export interface ProjectStats { total: number; done: number; in_progress: number; offen: number; percent_done: number; by_gewerk: StatsByGewerk[]; }
...
const ProjektStatistik: React.FC<ProjektStatistikProps> = ({ stats, projectId, className }) => {
  ...
  const [untilDate, setUntilDate] = useState<string>(() => formatDate(new Date()));
            ...
            percent_done:
              ...
              (s.tasks_total
                ? Math.round(((s.tasks_done || 0) / s.tasks_total) * 10000) /
                  100
                : 0),
            by_gewerk: Array.isArray(s.by_gewerk) ? s.by_gewerk : [],
          };
          setData(normalized);
        }
      ...
      <div className="bg-gradient-to-br from-gray-900 via-slate-800 to-gray-900 rounded-2xl p-6 shadow-2xl text-white border border-gray-700">
        <div className="flex items-center justify-between gap-4 mb-4">
          <h3 className="text-2xl font-bold text-cyan-400">
            📊 Aufgaben-Statistik{view.scopeLabel ? ` — ${view.scopeLabel}` : ""}
          </h3>
          ...
          <div className="flex items-center gap-3">
            ...
            <label className="flex items-center gap-2 text-sm">
              ...
              <input
                ...
                className="bg-slate-900 border border-slate-600 rounded px-2 py-1 text-gray-100 pr-10" // ⬅ pr-10 da ne preklapa tekst
              />
              <button
                type="button"
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[0.6fr_1.4fr] gap-4">
            ...
            <div className="space-y-4">
              <ul className="list-disc list-inside text-sm text-gray-200 space-y-1">
                <li><span className="text-cyan-300">Gesamt:</span> {view.total}</li>
                <li><span className="text-green-400">Erledigt:</span> {view.done}</li>
                <li><span className="text-yellow-400">In Bearbeitung:</span> {view.in_progress}</li>
                <li><span className="text-red-400">Offen:</span> {view.offen}</li>
                <li><span className="text-blue-300">Fertigstellungsgrad:</span> {view.percent_done}%</li>
              </ul>

              <div>
                <h4 className="text-lg font-semibold text-purple-300 mb-2">📁 Nach Gewerk:</h4>
                ...
                <ul className="space-y-0.5 leading-tight text-[13px]">
                  ...
                  <li>
                    ...
                    >
                      <span className="underline decoration-dotted">Gesamt (Reset)</span>
                    </button>
                  </li>
                  ...
                  {gewerkeSorted.map((g) => {
                    ...
                    return (
                      <li key={g.gewerk}>
                        <button
                          type="button"
                          onClick={() => setSelectedGewerk(active ? null : g.gewerk)}
                          className={
                            "text-left w-full px-2 py-0.5 rounded transition " + // ⬅ manje vert. paddinga
                            (active
                              ? "bg-cyan-900/40 text-cyan-200"
                              : "hover:bg-white/5 text-gray-300 hover:text-white")
                          }
                        ...
                        >
                          <span className="underline decoration-dotted">{g.gewerk}</span>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
              <div className="bg-black/20 p-4 rounded-xl shadow-inner border border-slate-700">
                <h4 className="text-center mb-2 text-sm text-gray-300">
                  📈 Kreisdiagramm{view.scopeLabel ? ` — ${view.scopeLabel}` : ""} (bis {formatDDMMYYYY(untilDate)})
                </h4>
              ...
              <div className="bg-black/20 p-4 rounded-xl shadow-inner border border-slate-700">
                <h4 className="text-center mb-2 text-sm text-gray-300">
                  📊 Balkendiagramm{view.scopeLabel ? ` — ${view.scopeLabel}` : ""} (bis {formatDDMMYYYY(untilDate)})
                </h4>
                ...
                <Bar
                  ...
                  options={{
                    ...
                    scales: {
                      x: { stacked: true, ticks: { color: "#cbd5e1" }, grid: { color: "#334155" } },
                      y: { stacked: true, beginAtZero: true, ticks: { color: "#cbd5e1" }, grid: { color: "#334155" } },
                    },
                  }}
                />
              </div>
